#pragma once
/*
 * Regroove Distortion - NTS-3 kaoss pad
 * RE-USES existing code from /rfx/effects/fx_distortion.c
...
public:
  ...
  inline void setParameter(uint8_t index, int32_t value) override final
  {
    ...
    {
    case PARAM_DRIVE:
      fx_distortion_set_drive(fx_, valf);
      break;

    case PARAM_MIX:
      fx_distortion_set_mix(fx_, valf);
      break;
  ...
  void init(float *allocated_buffer) override final
  {
    ...
    if (fx_)
    {
      fx_distortion_set_enabled(fx_, 1);
      fx_distortion_set_drive(fx_, 0.0f);
      fx_distortion_set_mix(fx_, 0.0f);
    }
  }
  ...
  void process(const float *__restrict in, float *__restrict out, uint32_t frames) override final
  {
    ...
    for (uint32_t i = 0; i < frames; i++)
    {
      ...
      fx_distortion_process_frame(fx_, &left, &right, sample_rate);
