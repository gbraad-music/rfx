...
__unit_callback void unit_gate_on(uint8_t velocity) {
  // RGSFZ is a polyphonic sampler that needs note information to select SFZ regions.
  // Gate callbacks don't provide note numbers, so they cannot be used.
  // Use unit_note_on() instead which provides the note parameter.
  (void)velocity;
}

__unit_callback void unit_gate_off() {
  // See unit_gate_on() comment - gate callbacks are not supported for polyphonic samplers.
  // Use unit_note_off() instead.
}
