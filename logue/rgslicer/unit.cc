...
__unit_callback void unit_gate_on(uint8_t velocity) {
  // RGSlicer is a polyphonic slicer that needs note information
  // Gate callbacks don't provide note numbers, so use unit_note_on() instead
  (void)velocity;
}

__unit_callback void unit_gate_off() {
  // See unit_gate_on() comment
}
