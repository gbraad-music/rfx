...
void rg909_synth_trigger_drum(RG909Synth* synth, uint8_t note, uint8_t velocity, float sample_rate) {
    ...
    switch (type) {
        case RG909_DRUM_BD: {
            ...
            // Amplitude envelope
            v->decay_env = 1.0f;
            float decay_time = 0.15f + synth->bd_decay * 0.3f;  // 150-450ms
        ...
        for (int i = 0; i < RG909_MAX_VOICES; i++) {
            ...
            switch (voice->type) {
                case RG909_DRUM_BD: {
                    ...
                    // Generate triangle wave (909 uses triangle, not sine)
                    ...
                    float triangle = fabsf(voice->noise_env * 4.0f - 2.0f) - 1.0f;

                    // Soften with diode-like clipping (makes it rounder like 909)
                    sample = tanhf(triangle * 1.5f) * 0.85f;

                    // Amplitude envelope: Attack (0-15ms) then Decay
                    float amp_env;
                    float attack_time = 0.015f;  // 15ms attack
                    if (voice->sweep_pos < attack_time) {
                        // Fast attack to build punch
                    ...
                    // Soft saturation (909 transistor distortion)
                    sample = tanhf(sample * 4.0f) * 1.0f;

                    // Attack click (very brief)
                    if (voice->sweep_pos < 0.0005f) {
                        sample += synth->bd_attack * 2.0f * (1.0f - voice->sweep_pos / 0.0005f);
                    }
